...
void main() {
  group('a written message', () {
    test('the model has something to call, with the words to put in it', () {
      final send = GeminiAssistantService.functionDeclarations.firstWhere(
        (f) => f.name == 'send_caretaker_message',
      );
    ...
    test('a message with content is left to Gemini, not eaten locally', () async {
      ...
      final intent = LocalIntentMatcher.match(
        'tell my caretaker I will be home late tonight',
        AppLanguage.english,
      );
      expect(
        intent,
        isNull,
        reason:
            'the message text is Gemini\'s to extract — see the class comment',
      );
    });
  ...
  group('a voice message', () {
    test('the model has something to call', () {
      final names = GeminiAssistantService.functionDeclarations.map(
        (f) => f.name,
      );
    ...
    for (final phrase in [
      ...
      test('"$phrase" is matched locally', () {
        ...
        expect(
          LocalIntentMatcher.match(phrase, AppLanguage.english)?.name,
    ...
    for (final phrase in ['ভয়েস বার্তা পাঠাও', 'record kore pathao']) {
      test('"$phrase" is matched too — item 54', () {
        expect(
          LocalIntentMatcher.match(phrase, AppLanguage.bangla)?.name,
          'record_caretaker_voice_memo',
        );
      });
    }
  ...
  group('caretaker composer voice commands', () {
    test('bare message request arms the next utterance', () {
      expect(
        LocalIntentMatcher.match(
          'send message to caretaker',
          AppLanguage.english,
        )?.name,
        'arm_caretaker_message',
      );
      expect(
        LocalIntentMatcher.match(
    ...
    test(
      'an explicit snapshot to the caretaker sends rather than only describes',
      () {
        expect(
          LocalIntentMatcher.match(
            "send what's in front of me to caretaker",
            AppLanguage.english,
          )?.name,
  ...
  group('the two directions stay distinguishable', () {
    ...
    test('an emergency that mentions a voice note is still an emergency', () {
      final intent = LocalIntentMatcher.match(
        'help me i have fallen record a message',
        AppLanguage.english,
      );
